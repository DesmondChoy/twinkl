...
def _evidence_from_decisions(
    ...
    *,
    entry_texts: dict[tuple[str, int], str],
    limit: int = 3,
    evidence_policy: Literal["complete", "historical"] = "complete",
) -> list[EvidenceSnippet]:
    """Keep full displayed text for the capped set of selected decisions."""
    snippets: list[EvidenceSnippet] = []
    for decision in sorted(decisions, key=lambda row: row.t_index)[-limit:]:
        key = (decision.date, decision.t_index)
        if evidence_policy == "historical":
            excerpt = (
            ...
            excerpt = entry_texts.get(key, "")
            if not excerpt.strip() and decision.verdict == "conflict":
                excerpt = decision.evidence_quote.strip()
        if not excerpt or (evidence_policy == "complete" and not excerpt.strip()):
            continue
        snippets.append(
            EvidenceSnippet(
                date=decision.date,
                ...
                excerpt=(
                    _truncate_excerpt(excerpt)
                    if evidence_policy == "historical" else excerpt
                ),
            )
        )
    return snippets
    ...
    if not evidence:
        evidence = [
            EvidenceSnippet(
                ...
                dimensions=[],
                excerpt=(
                    _truncate_excerpt(entry.text)
                    if evidence_policy == "historical" else entry.text
                ),
            )
...
def validate_weekly_digest_narrative(
    ...
    *,
    ...
    checks = [
        ...
        ValidationCheck(
            name="length",
            passed=minimum_words <= word_count <= max_words,
            details=(
                f"Combined response length is {word_count} words "
                + (
                    f"(target {minimum_words}-{max_words})."
                    if minimum_words else f"(maximum {max_words}; no minimum)."
                )
            ),
        ),
    ]
    ...
    if validation_policy != "historical":
        ...
        checks.extend([
            ValidationCheck(
                ...
                details=(
                    "Weekly Mirror preserves an exact quotation from weekly evidence."
            ...
            ValidationCheck(
                name="nonempty_fields",
                passed=all(text.strip() for text in narrative.model_dump().values()),
                details="Return nonempty text in all three response fields.",
            ),
            ValidationCheck(
    ...
    if validate_voice:
        ...
        if voice_version == "4.5":
            calendar = (
                r"(?:January|February|March|April|May|June|July|August|"
                r"September|October|November|December|Jan|Feb|Mar|Apr|Jun|"
                r"Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.?\s+\d{1,2}\b|"
                r"\d{1,2}(?:st|nd|rd|th)?\s+(?:of\s+)?(?:January|February|"
                r"March|April|May|June|July|August|September|October|November|December)\b|"
                r"\d{4}-\d{2}-\d{2}\b|"
                r"(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\b"
            )
            date_opening = re.match(
                rf"^(?:(?:on|by|as of)\s+)?(?:{calendar})", narration[0], re.IGNORECASE
            )
            writing_commentary = re.search(
                r"\b(?:across|in) (?:the |your )?(?:recent |journal )?entries\b|"
                r"\byou (?:also )?(?:named|identified|noted) the "
                r"(?:calculation|tension|pattern|contrast)\b",
                " ".join(narration), re.IGNORECASE,
            )
            checks.append(ValidationCheck(
                name="natural_reflection_voice",
                passed=date_opening is None and writing_commentary is None,
                details=(
                    "Open with the person's situation rather than a date or weekday. "
                    "Preserve chronology using earlier or later where needed; "
                    "do not alter the source facts or quotations."
                    if date_opening else
                    "Describe the person's experiences directly. Remove commentary "
                    "about the entries or how the person named a calculation, "
                    "tension, pattern, or contrast. Keep required quotations intact."
                    if writing_commentary else
                    "Response avoids date-led openings and writing-process commentary."
                ),
            ))
