...
import endedReplayJson from "../public/scenarios/ended-sook-yin.json";
...
const fixture = validateExperienceInspectFixture(endedReplayJson);
...
describe("ended Drift explanation", () => {
  ...
  it("distinguishes an earlier ending from the selected week's decisions", () => {
    render(<DriftStateExplanation {...propsForWeek(3)} />);
    ...
    expect(screen.getByRole("button", {
      name: /Read Journal Entry from (?=.*21)(?=.*Feb)(?=.*2025)/,
    })).toBeTruthy();
  });

  it("does not expose a later ending while Drift is still active", () => {
    render(<DriftStateExplanation {...propsForWeek(1)} />);
  ...
  it.each(["abstain", "failed_review", "missing_decision"])(
    ...
    (evidence) => {
      ...
      props.weeklyReviewerDecisions = props.weeklyReviewerDecisions.flatMap((decision) => {
        if (decision.t_index !== 4) return [decision];
