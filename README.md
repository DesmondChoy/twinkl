...
**Current drift contract:** Drift v1 means a sustained conflict episode: the same declared core value receives a stored five-pass Judge consensus `-1` label on two adjacent journal entries. Other dimensions do not offset it. The offline benchmark evaluates adjacent-pair mean `P(-1)` under an uncertainty ceiling, but no scorer is promotion-ready: the LLM arms detect 0/5 frozen consensus episodes versus 10/10 locked author-designed episodes, while the MLP arms detect only 1–2/10 designed episodes. A procedurally metadata-blinded Codex audit found that the frozen references are not a stable student-visible promotion surface: 1/5 frozen cases qualified, versus 10/10 designed positives and 0/10 controls. These AI-audit results are not human ground truth; secondary delivery/context fields differed without affecting qualification. `twinkl-v8pb` now repairs the target contract before production trigger wiring in `twinkl-a2w`. The current runtime remains an experimental predecessor that emits `stable`, `crash`, `rut`, `evolution`, and `high_uncertainty`. See [`docs/evals/drift_detection_eval.md`](docs/evals/drift_detection_eval.md) and the [`twinkl-wq9p` report](logs/experiments/reports/experiment_review_2026-07-10_twinkl_wq9p.md).
...
| Weekly Coach validation depth | ⚠️ Partial | Digest generation, runtime artifacts, and demo review flow exist; selected-v1 production wiring is blocked by the unresolved benchmark result, and narrative evaluation remains incomplete |
...
| Drift scorer validity and production wiring | 🧪 Experimental | The strict reference builder, soft-evidence benchmark, threshold tuning, metrics, locked designed holdout, and procedurally metadata-blinded Codex audit are complete. No scorer is promotion-ready: LLM 0/5 frozen versus 10/10 designed; MLP 1–2/10 designed. The audit qualified 1/5 frozen cases, 10/10 designed positives, and 0/10 controls; it is AI diagnostic evidence, not human ground truth. `twinkl-v8pb` must repair the student-visible target contract and establish an untouched promotion surface before production wiring. |
