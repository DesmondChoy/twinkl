...
**Current drift contract:** Drift v1 means a sustained conflict episode: the same declared core value receives a stored five-pass Judge consensus `-1` label on two adjacent journal entries. Other dimensions do not offset it. The offline benchmark now evaluates adjacent-pair mean `P(-1)` under an uncertainty ceiling, but no scorer is promotion-ready: the LLM arms detect 0/5 frozen consensus episodes versus 10/10 locked author-designed episodes, while the MLP arms detect only 1–2/10 designed episodes. Human review is required before the selected rule is wired into the weekly Coach. The current runtime remains an experimental predecessor that emits `stable`, `crash`, `rut`, `evolution`, and `high_uncertainty`. See [`docs/evals/drift_detection_eval.md`](docs/evals/drift_detection_eval.md) and the [`twinkl-wq9p` report](logs/experiments/reports/experiment_review_2026-07-10_twinkl_wq9p.md).
...
| Drift scorer validity and production wiring | 🧪 Experimental | The strict reference builder, soft-evidence benchmark, threshold tuning, metrics, and locked designed holdout are complete. No scorer is promotion-ready: LLM 0/5 frozen versus 10/10 designed; MLP 1–2/10 designed. Human review and a scorer/contract decision are required before production wiring. |
