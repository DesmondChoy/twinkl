...
**Current drift contract:** Drift v1 means a sustained conflict episode: the same declared core value receives a stored five-pass Judge consensus `-1` label on two adjacent journal entries. Other dimensions do not offset it. The runtime target is rolling soft `P(-1)` evidence under uncertainty gating, delivered through the weekly Coach digest; it is not implemented yet. The current code path is an experimental predecessor that still emits `stable`, `crash`, `rut`, `evolution`, and `high_uncertainty`. The review UI also compares six exploratory rule-based detectors against either Judge labels or Critic predictions; its detector-vote count is not the five-pass Judge reference. See [`docs/drift/trajectory_eda.md`](docs/drift/trajectory_eda.md), [`docs/evals/drift_detection_eval.md`](docs/evals/drift_detection_eval.md), and [`docs/demo/review_app.md`](docs/demo/review_app.md).
