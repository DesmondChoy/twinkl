...
ComparisonPromptVersion = Literal["1.0", "1.1"]
PROMPT_VERSION: ComparisonPromptVersion = "1.1"
COMPARISONS_PATH = "src/demo/coach_digest_comparisons.json"


def _prompt_templates(
    version: ComparisonPromptVersion,
) -> tuple[str, str, Literal["4.4", "4.5"]]:
    if version == "1.0":
        return (
            "versions/weekly_digest_coach_v4_4",
            "versions/demo_coach_nsm_comparison_v1_0",
            "4.4",
        )
    if version == "1.1":
...
def render_demo_comparison_messages(
    ...
    *,
    ...
    base_name, extension_name, voice_version = _prompt_templates(prompt_version)
    if (
        get_prompt_metadata(base_name)["version"] != voice_version
        or get_prompt_metadata(extension_name)["version"] != prompt_version
    ):
        raise ValueError("Coach comparison prompt template version changed")
    instructions, input_data = render_digest_messages(digest, prompt_name=base_name)
    marker = "\n\nReturn JSON with exactly these keys:"
    if instructions.count(marker) != 1:
        raise ValueError("Coach comparison output boundary changed")
    extension = str(load_prompt(extension_name).render()).strip()
    ...
    *,
    ...
    checks = [
        ValidationCheck(
            ...
            details="In weekly_mirror, copy a short continuous phrase character-for-"
            "character from the excerpt portion of an evidence_lines item and put "
            "it in double quotation marks. A quotation from north_star_context, "
            "including exact_quote, does NOT satisfy this requirement unless it "
            "also appears in evidence_lines. Describe the selected action separately "
            "if needed. For this revision, take the required quotation ONLY from "
            "the FIRST evidence_lines item's excerpt, choosing 3-12 consecutive "
            "words without prohibited jargon. Use the selected source only as an "
            "unquoted paraphrase. Do not reword, recapitalize, or splice "
            "the quoted phrase."
...
async def generate_demo_comparison_diagnostic(
    ...
    *,
    ...
    validation = validate_demo_comparison_narrative(
        digest, narrative, context, prompt_version=prompt_version
    )
...
def validate_saved_comparison(
    ...
    if (
        comparison.without_north_star.prompt_version
        != comparison.with_north_star.prompt_version
    ):
        raise ValueError("Coach comparison arms use different prompt versions")
    for arm, arm_context in (
        (comparison.without_north_star, None),
        (comparison.with_north_star, context),
    ):
        base_prompt = render_demo_comparison_prompt(
            digest, arm_context, prompt_version=arm.prompt_version
        )
        prompt = render_demo_comparison_prompt(
            digest,
            arm_context,
            repair_requirements=arm.repair_requirements,
