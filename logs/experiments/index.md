...
>
> **Latest recall-aware checkpoint replay:** [`reports/experiment_review_2026-06-06_twinkl_t2r0.md`](reports/experiment_review_2026-06-06_twinkl_t2r0.md) does **not** change the frontier. A `0.02` validation-QWK window would have selected higher-`recall_-1` epochs for `run_048` (`+0.060` recall at `-0.014` QWK) and `run_049` (`+0.107` recall at `-0.011` QWK), while `run_050` already selected the same epoch. But those alternate epochs were not serialized as checkpoints, so they cannot be scored on the fixed corrected split from current artifacts. Treat the replay as a lead for exact reruns with candidate-checkpoint retention, not a promotion result.
>
