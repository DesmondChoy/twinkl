...
>
> **Weekly Drift Reviewer model comparison (2026-07-14):** [`twinkl-52zz`](reports/experiment_review_2026-07-14_twinkl_52zz_model_comparison.md) compared `gpt-5.4-mini-2026-03-17` with `gpt-5.6-luna`, both at reasoning effort `none`, on all 951 persona-weeks in the complete 204-persona development data. Luna raised median Drift recall from `0.167` to `0.476`; the paired delta was `+0.286` with 95% trajectory-bootstrap interval `[+0.158, +0.425]`. It also raised median false Drift alerts from 5 to 13; the paired delta was `+9` with interval `[+2, +17]`. Median Drift precision stayed similar (`0.583` vs `0.606`) and coverage rose (`0.740` vs `0.777`). The preregistered no-added-false-alert rule mechanically retained mini, but the user explicitly accepted the trade-off and selected Luna at reasoning effort `none` for the reasoning-effort follow-up. This setup remains that follow-up's frozen baseline. The [OpenAI cost receipt](artifacts/twinkl_52zz_model_comparison_20260714/receipts/README.md) records `$12.01916515` for the whole OpenAI project-day, below the `$15` cap; it does not isolate the comparison's billed cost. This is development evidence, not deployment approval.
>
> **Luna reasoning-effort follow-up (2026-07-14; decision updated 2026-07-15):** [`twinkl-52zz`](reports/experiment_review_2026-07-14_twinkl_52zz_luna_low.md) compared Luna reasoning effort `low` with the frozen `none` baseline on the same 951 prompts, all 204 personas, and three repeats. `low` raised median Drift recall from `0.476` to `0.548` and cut median false Drift alerts from 13 to 4, while coverage fell from `0.777` to `0.637`. The paired recall delta was `+0.071` with interval `[-0.071, +0.205]`; false Drift alerts fell by 9 `[-16, -3]`, while coverage fell by `0.140` `[-0.188, -0.092]`. The preregistered coverage gate mechanically retained `none`; the later approved hierarchy ranks Drift recall first, false Drift alerts second, and coverage as diagnostic. Luna `low` is therefore the development Weekly Drift Reviewer, and the study stops before `medium`. Detailed token receipts produce an `$8.371455` full-input calculation and a `$6.9603028` cache-aware calculation. This is development evidence, not deployment approval.
>
> **Recall-first VIF Critic checkpoint policy (2026-07-19):** [`twinkl-6mrt`](reports/experiment_review_2026-07-19_twinkl_6mrt_recall_first_checkpoint_selection.md) implements `recall_first_qwk_guarded_v1`. Validation checkpoints must clear QWK `0.3712`, the repaired-Security family median selected validation QWK (`0.3912`) minus `0.02`; eligible checkpoints are then ranked by `recall_-1`. Historical QWK-first selection remains available as `qwk_then_recall_guarded`. Retrospective development-trace replay cannot recover unsaved model states, so it does not rewrite historical Runs. Among comparable saved repaired-Security checkpoints, `run_060` is the offline nomination because it has the highest stored selection `recall_-1` (`0.4984`) while clearing the QWK floor (`0.3943`). This is not fresh final-test evidence or deployment approval.
>
> **Adopted VIF scope and metric policy (2026-07-12):** [`twinkl-752`](../../docs/vif/05_capstone_scope_decision.md) defines the VIF Critic primarily as a Conflict screener. Per-Journal-Entry `recall_-1` is the primary model-development metric; Drift recall is the future product metric; QWK and `+1` recall are diagnostics. No numerical Conflict precision or false-alert tolerance is adopted, so recall-focused development cannot support deployment approval by itself. The ternary ten-value output remains. The user-facing path uses the Weekly Drift Reviewer without VIF Critic input; the VIF Critic remains in the offline review-and-retrain path.
>
...
>
> **Complete development review (2026-07-14):** [`twinkl-qtwz`](reports/experiment_review_2026-07-14_twinkl_qtwz_complete_development_review.md) reviewed the 186 cases outside the frozen 106-case union: 1,483 Journal Entry/Core Value combinations, 1,412 initial agreements, and 71 disagreements resolved by a blinded LLM-Judge adjudicator with no remaining Uncertain LLM-Judge Conflict Labels. The complement contains nine Drifts across eight Drift trajectories. The complete 292-case development analysis therefore contains 42 Drifts across 36 Drift trajectories. All nine newly found Drifts have historical training provenance, so VIF Critic results on them are in-sample. This is AI-reviewed synthetic development evidence, not human validation, prevalence, a fresh final test, or deployment approval.
>
...
| 051 | BalancedSoftmax | nomic-256d | 1 | 64 | 0.3 | balanced_softmax | 23454 | 19.3 | 0.274 | 0.777 | 0.365 | 0.366 | 0.767 | 0.395 | 0.044 | 0.062 | runs/run_051_BalancedSoftmax.yaml |
| 052 | BalancedSoftmax | nomic-256d | 1 | 64 | 0.3 | balanced_softmax | 23454 | 19.3 | 0.251 | 0.804 | 0.374 | 0.373 | 0.774 | 0.397 | 0.047 | 0.058 | runs/run_052_BalancedSoftmax.yaml |
| 053 | BalancedSoftmax | nomic-256d | 1 | 64 | 0.3 | balanced_softmax | 23454 | 19.3 | 0.285 | 0.775 | 0.403 | 0.382 | 0.724 | 0.469 | 0.069 | 0.086 | runs/run_053_BalancedSoftmax.yaml |
...
The reasoning-effort follow-up ran 2,853 `gpt-5.6-luna` calls over the same 951
prompts and complete 204-persona development data as the frozen `none`
baseline. Reasoning effort `low` found a median 23/42 known Drifts (`0.548`
recall) with 4 false Drift alerts, versus 20/42 (`0.476`) with 13 false Drift
alerts for `none`.

The paired Drift-recall delta was `+0.071` with 95% trajectory-bootstrap
interval `[-0.071, +0.205]`. The false Drift alert delta was `-9` with interval
`[-16, -3]`, while coverage fell by `0.140` with interval `[-0.188, -0.092]`
and median delay rose by 2.5 days. The preregistered coverage gate mechanically
retained `none`, but the approved metric hierarchy ranks Drift recall first,
false Drift alerts second, and coverage as diagnostic. Select `low` for
development and stop before `medium`. Detailed cache accounting gives a
`$6.9603028` token calculation for `low`, versus `$8.371455` if every input
token is charged at the full rate. Full details:
[`reports/experiment_review_2026-07-14_twinkl_52zz_luna_low.md`](reports/experiment_review_2026-07-14_twinkl_52zz_luna_low.md).
...
`none` for the bounded reasoning-effort follow-up. The exact setup remains that
follow-up's frozen baseline; the later `low` result above supersedes it as the
development selection. Full details:
...
Two isolated Codex LLM-Judge lanes reviewed the 186 cases outside the frozen
106-case union: 1,483 Journal Entry/Core Value combinations in seven hash-bound,
blinded packets. They agreed on 1,412 decisions (`95.2%`); a third blinded
LLM-Judge adjudicator resolved all 71 disagreements without an Uncertain
LLM-Judge Conflict Label. The
complement contains nine Drifts across eight Drift trajectories.

Combining the immutable 106-case labels with the new complement produces a
complete 292-case development analysis with 42 Drifts across 36 Drift
trajectories. The earlier bounded selection contained 33/42 Drifts; the other
nine are selection misses. All nine have historical training provenance, so
future VIF Critic results on them are in-sample. The earlier `twinkl-752.5`
metrics remain 106-case results and were not recomputed here. No direct API
calls were made. Full details:
[`reports/experiment_review_2026-07-14_twinkl_qtwz_complete_development_review.md`](reports/experiment_review_2026-07-14_twinkl_qtwz_complete_development_review.md).
...
positions, with identity, split, prior labels, VIF Critic outputs, and expected
outcomes removed. The packet included the versioned Conflict rubric and Core
Value definitions. It returned three Conflict labels and one non-Conflict
label; the `5943c186:hedonism` Conflict and `3cfa2ebf:universalism`
non-Conflict are low confidence. None forms an adjacent Conflict pair.

The `twinkl-752.4` cohort is therefore 104/104 resolved, with the same 31 Drift
episodes across 26 Drift trajectories. Adding the two earlier episodes omitted
by candidate mining produces a 106/106-resolved known-development union with
the same 33 episodes across 28 Drift trajectories. This removes missing labels
but does not make the AI-reviewed union a final test or decide whether the MLP
belongs in the runtime. Full details:
[`reports/experiment_review_2026-07-14_twinkl_752_5_opus_null_resolution.md`](reports/experiment_review_2026-07-14_twinkl_752_5_opus_null_resolution.md).

### 2026-07-13 — Full legacy-discoverable Drift review expands the development reference (`twinkl-752.4`)

The review froze 52 candidate trajectories found by either legacy label source
and 52 unique-person matched legacy-negative controls. Two packet-only Codex
lanes reviewed all 874 entries and agreed on 849 (97.1%). A third blinded
adjudicator resolved 21 of 25 disagreements; four entries and their four
trajectories were left uncertain in the original artifact. The Opus follow-up
above later resolved them.

The review found 31 maximal Drift episodes across 26 resolved trajectories.
...
episodes came from the former final-test split; they belong in the primary
development analysis, with provenance reported as a subgroup. After the Opus
follow-up, 22/44 development-reference candidates and 1/44 controls contain
...
review, candidate mining, and retraining. The 2026-07-17 scope decision later
removed user-facing candidate confirmation from the remaining capstone work.
Full details:
