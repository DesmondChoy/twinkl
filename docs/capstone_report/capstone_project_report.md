...
| North Star Moment | Offline feasibility prototype; application integration blocked | Retrieval passed its label-proxy gate, but independent AI reference review rejected seven of 19 selected quotations. No NSM card, live onboarding integration, or browser validation is claimed. |
...
*Table 3. Evidence datasets and experiment setups used for the three investigations and implementation objective. Cases count Persona/Core Value histories; Persona-week prompts count reviews at weekly cutoffs. Known Drifts apply only to Investigation 3.*
...
Coach Digest receives structured Weekly Drift Detection output. It must cite relevant Journal Entry text, avoid prescriptive instructions and unsupported current-state claims, and ask one open question. Under the `more_reflection_needed` policy, it states the evidence limit without deciding whether Drift exists. Coach Digest Validations check that quotes match the cited Journal Entries and screen for score jargon, raw Schwartz value names in user-facing text, unsupported current-state claims, and responses outside the allowed length range.
...
### 3.9 North Star Moment: a gated feasibility extension

On 5 September 2026, the project examined North Star Moment (NSM) as a separate extension to the existing application. The proposed card would quote an earlier completed action supporting the affected Core Value after an eligible closed-week result. It would require the writer's own action, an exact quotation, eligible chronology, and no conflicting context for that Core Value. Its specification required a local retrieval test (Phase 0A) before paid development review (Phase 0B) and required stopping dependent work if either phase failed its acceptance criteria. The extension was therefore evaluated without changing Coach Digest or treating an illustrative quotation as evidence that the application feature worked.

Before retrieval tuning, the approved evaluation policy reserved eight complete non-demo Persona histories, containing nine Drift episodes, from the existing 35-Persona, 42-Drift corpus. Seed 20260905 and SHA-256 identifier ordering assigned the groups; 27 development Personas supplied 33 episodes, with all five saved Persona demos excluded from the reservation. These histories were reserved specifically for NSM, within a corpus already used in earlier Twinkl studies. The fixed Nomic encoder used 256-dimensional normalized embeddings, a query containing only the Core Value phrase and approved definition, and original Journal Entries dated no later than Drift onset and preceding it in stored entry order. Legacy nudge responses lacked independent evidence of when they became available and were excluded. Persisted positive LLM-Judge VIF Labels (`+1`) served only as the Phase 0A retrieval proxy: they identified likely supportive entries for testing the ranking, rather than verifying the suitability of a quotation. Their possible dependence on excluded response text limits that proxy.

Phase 0B froze all 33 development cases, each comprising one Drift episode and its eligible earlier Journal Entries, and selected the first supportive result among the top three retrieved sources after strict code validation. OpenAI `gpt-5.6-luna` at no reasoning effort performed this review, while Google `gemini-3.5-flash` at low thinking independently reviewed every eligible source to establish task-specific reference decisions and no-valid-example cases. Four selected quotations differing from the primary reference quotation received a predefined exact-candidate reference check. Reference rejection or abstention counted as incorrect, and unresolved review could not approve a quotation. This cross-provider procedure supplies AI reference evidence, not human validation. Prompts, input and code hashes, raw JSON, validation outcomes, model identifiers, usage, and costs were retained.

The implemented experimental architecture separates source filtering and pinned encoding, the Pydantic review contract, and a budgeted provider adapter. The adapter disables implicit SDK retries, reserves a conservative attempt cost in an atomically updated ledger, records actual usage, coalesces repeated requests, and permits at most one explicit retry. The approved limits were US$20 total and US$0.25 per attempt. Future application hosting was scoped to one lazily loaded CPU encoder in the existing backend, with bounded concurrency outside the asynchronous event loop; this arrangement avoids another deployment but still requires container and memory verification. It was not implemented after Phase 0B failed. Appendix C provides a source-level walkthrough of the resulting offline evidence.
...
![Weekly Drift Reviewer operating points on AI-reviewed synthetic development data. Each marker has an exact trajectory-coverage label; marker area carries no additional quantity. Twinkl retains low reasoning effort as the fixed capstone contract.](images/weekly-drift-tradeoff.png){#fig-weekly-tradeoff fig-alt="Scatter plot of false Drift alerts against median Drift recall for no, low, medium, high, and xhigh reasoning effort, with trajectory-coverage labels."}
...
### 4.5 North Star Moment: retrieval feasibility did not establish semantic reliability

The local retrieval gate passed at k=3 (the top three retrieved Journal Entries): the ranking included an earlier positive-label Journal Entry for 21 of 22 eligible episode-specific development histories (95.5%), compared with 13/22 at k=1 and 22/22 at k=5. The complete measurement took 5.482 seconds, including 2.174 seconds to encode 119 unique original Journal Entries, and reached 1,584.1 MiB peak resident memory on the recorded macOS CPU environment. These measurements establish local encoder feasibility rather than live-service latency or Railway container capacity.

The task-specific AI reference exposed a different limit. Only 12 of 19 selected quotations were accepted (63.2%), and correct omission occurred in five of nine nonempty histories for which exhaustive reference review found no valid example (55.6%). Five additional histories had no eligible earlier writing and correctly required no model call. Against the task-specific reference, top-three retrieval recall was 17/19 (89.5%). This measure uses a different target and denominator from the persisted-label proxy, so positive LLM-Judge VIF Labels could not substitute for the stricter NSM task definition. @fig-north-star-feasibility separates the passing proxy gate from the failed semantic criteria.

![NSM development feasibility results. The left panel compares persisted-label proxy recall at three retrieval depths against the 90% gate. The right panel shows quotation precision and correct omission against the zero-error criteria; the denominators describe different tasks.](images/north-star-feasibility.png){#fig-north-star-feasibility width=100% fig-alt="Proxy retrieval recall is 13 of 22 at k1, 21 of 22 at k3, and 22 of 22 at k5. Independent AI reference review accepts 12 of 19 selected quotations and confirms correct omission in 5 of 9 nonempty no-valid-example histories. The semantic criteria fail."}

The seven rejected selections comprised three `wrong_value`, two `ambiguous`, and two `same_value_conflict` reference decisions. Exact source matching did not prevent a statement of financial stability without a supporting action, an enjoyable action selected despite conflicting context, or a helpful act whose relation to Universalism the reference rejected. These classifications describe the AI reference's interpretation; they do not establish a human-adjudicated error taxonomy. Retained quotations passed the audited identity, chronology, and exact-source checks, illustrating that mechanical validity and semantic support address different failure modes.

One case failed after its allowed retry because both responses paired `abstain` with a reason permitted only for `not_supportive`. These were two contract-invalid responses among 29 OpenAI attempts (6.9%), exceeding the specification's 5% unexpected-failure ceiling despite successful transport. All 32 Gemini attempts completed validly. Across 61 attempts, returned usage implied US$0.2062 at the frozen rates. Median attempt latency was 2.073 seconds for OpenAI and 2.271 seconds for Gemini; these measurements exclude encoding and the application workflow. The offline review selected quotations in 19/33 episodes, a descriptive experimental coverage rate rather than the unimplemented application's closed-week priority behavior. Retrieval-only source precision was 7/28; because it grades the first source rather than an independently checked quotation, its difference from quotation precision is not reported as a matched verification effect.

**Feasibility finding:** The fixed retrieval configuration passed its proxy gate, but semantic precision, omission, and contract-invalid response frequency failed the adopted development criteria. Accepted offline examples from Wei Jun and Marc satisfied the demonstration-example condition without resolving those failures. Dependent integration stopped: the five saved Persona bundles, live onboarding, Experience, and Inspect retain their existing behavior. The NSM-specific and affected backend regression targets passed 249 tests, including 110 new focused tests, with Ruff and isolated MyPy passing. These checks verify the experimental implementation, not the proposed browser experience. Narrow- and wide-width NSM browser QC, source-change invalidation, quotation expansion, source navigation, Inspect interactions, and the reserved final evaluation remain blocked.
...
The NSM extension adds a separate development-evidence limitation. Thirty-three episodes can share writing within a Persona, so their counts do not represent independent participants, and the Gemini reference may itself misinterpret context or Core Value specificity. The eight NSM-reserved histories remain unevaluated because the development gate failed. The experiment therefore supports withholding the proposed card under the adopted criteria, without establishing a final-test error rate or ruling out a revised development approach. It also provides no evidence for live source invalidation, browser usability, human usefulness, or deployed encoder capacity.
...
The failed NSM feasibility gate argues for examining semantic disagreements and the omission rule before adding application lifecycle or interface complexity. Further development must retain the distinction between proxy retrieval, task-specific AI review, the NSM-reserved evaluation histories, and subsequent human or browser validation; neither an exact quotation nor unused paid budget establishes readiness to integrate the feature.

## AI Tool Declaration

We used OpenAI Codex to inspect repository evidence, create report figures, capture local application screenshots, and help draft and edit this paper. Figure inputs include the separately dated NSM evidence snapshot identified in Appendix A. We checked numerical claims against the named evaluation reports, current code, and stored run records. The authors remain responsible for study design, interpretation, source verification, and submitted text.

The project also used language models for synthetic Journal Entry generation, LLM-Judge VIF Labels, LLM-Judge Conflict Labels, Weekly Drift Reviewer Decisions, Coach Digest generation, Coach Digest Evals, and NSM development and reference review. Codex also audited the NSM receipts and helped document the failed gate. Each use and its evidence limit are stated in the relevant method or result section.
...
Sections 3.9 and 4.5 and Appendix C add a separately dated 5 September 2026 NSM supplement. Its frozen code and raw evidence are preserved in commit [`f3030c8d`](https://github.com/DesmondChoy/twinkl/tree/f3030c8deb9400685185e7c620bda53a6f58e8aa); the older snapshots do not contain them. The [Phase 0A report](../../logs/experiments/reports/north_star_phase0_20260905/README.md), [Phase 0B report](../../logs/experiments/reports/north_star_phase0b_20260905/README.md), and [independent code/evidence audit](../../logs/experiments/reports/north_star_phase0b_20260905/audit.md) explain the split, policy, inputs, receipts, and execution hashes. The original JSON evidence and experiment code remain unchanged after audit. The audit identified preparation and retry-replay limitations for subsequent runs; these do not alter the verified historical result. This supplement reports the frozen experiment rather than subsequent NSM development revisions.
...
## Appendix C. NSM Offline Evidence Walkthrough

The NSM walkthrough concerns saved experimental receipts, not an implemented browser card. In Wei Jun's Universalism episode, the reviewer rejected the earlier-ranked sources before selecting entry 7: “Helped two new guys file their claims.” The full Journal Entry describes a worker-center morning, the quotation matches the source exactly, and the independent reference returned the same quotation. Its stored order precedes onset at entry 8. Marc's Power episode similarly selected entry 1: “Ran the quarterly review myself today, no slides from anyone else, just my numbers, my narrative.” The reference accepted the same passage before onset at entry 5. Marc's configured highlighted demo week occurs after that Active Drift ends; this earlier accepted source does not justify a card at the later highlighted week.

Noor and Lukas each contribute two development episodes. Their first episodes correctly omitted a selection because exhaustive AI reference review found no accepted earlier source. Their second episodes also omitted, but the reference found support outside the top three retrieved sources: Noor's entry 7 and Lukas's entries 4 and 5. Meera contributes no Drift episode to this manifest, and her saved highlighted week is configured as No Active Drift. That configuration establishes intended non-triggering behavior only; it is neither a completed NSM browser test nor a semantic no-example case.

The [derived audit](../../logs/experiments/reports/north_star_phase0b_20260905/audit.md) links these receipts, the seven semantic disagreements, the bounded retry failure, and the frozen-hash checks. No screenshot from the existing application is presented as NSM evidence. Fresh onboarding through closed-week NSM review, narrow- and wide-width card interactions, quotation expansion, source navigation, source-change invalidation, and NSM Inspect remain unexecuted because the feasibility gate stopped integration. The existing application walkthrough in Section 4.4 retains its original scope.
