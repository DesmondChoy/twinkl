...
| Coach Digest | Experimental | Five current v4 Run 1 replay responses passed Coach Digest Validations after fresh Luna-none generation. No new Coach Digest Evals or human review was performed. The five August responses and same-model AI scores retain their historical input provenance. |
| North Star Moment | AI evaluation integrated in saved replay | The 501-week comparison and targeted v4 Run 1 update favour full eligible history for Card precision and Opportunity recall. The saved replay uses completed full-history outcomes for five Personas and 27 reviewed weeks. These are AI assessments of synthetic histories. Live generation requires a fresh budget, and the final source-level walkthrough remains pending. |
...
North Star Moment (NSM) is implemented to quote a reported action or already-made choice supporting a confirmed Core Value after an eligible closed-week result. Active Drift uses writing from before onset; No Active Drift prefers writing from the reviewed week and otherwise uses an earlier reference point. The implementation supports source-checked cards and Inspect records in both frontend paths. Saved replay now uses the completed full-history outcomes for five selected Personas and 27 reviewed weeks from the targeted v4 Run 1 record, whereas live generation remains unavailable until a fresh budget is configured.
...
The original experiment used Weekly Drift v2 repeat 1. A [separate targeted update](../../logs/experiments/reports/north_star_v4_run1_20260907/report.md) adopted the completed v4 Run 1 outputs, whose prompt retains the existing Core Value definition and motivation fields. An impact audit identified 38 changed effective contexts across 22 Personas, comprising 29 development weeks and nine final weeks. Both NSM methods were recomputed for those contexts, while 463 unchanged contexts retained their observations, including failures and unresolved judgments. Individual receipts were reused only when their complete requests, experimental roles, repeat identities, settings, and evaluation contracts matched. The original 20-case consistency sample was unaffected and retained. All 501 cases were then regraded; these results constitute a targeted update with retained observations rather than a fresh independent experiment.
...
Table 4.5. AI-reviewed NSM results across 391 development and 110 qualified-final weeks.
...
Sections 3.9 and 4.5 use the later [NSM methodology](../north_star/nsm_experiment_methodology.md) and [versioned targeted-update record](../../logs/experiments/reports/north_star_v4_run1_20260907/nsm_experiment.json). Those results are separate from the earlier core evidence snapshots below. Their original and updated inputs, generation receipts, corrected judgments, and code hashes are preserved in the linked records; the targeted v4 Run 1 update is the latest NSM experiment result to use for current reporting, while the original v2 repeat-1 record remains historical evidence. Appendix C retains the boundary between completed AI evaluation and the pending source-level application walkthrough.
...
The [September replay refresh](../../logs/experiments/reports/demo_v4_run1_20260907/report.md) generated five accepted responses from the current v4 Run 1 key-week inputs, using `gpt-5.6-luna` at reasoning effort `none` and prompt version `4.2`. Seven calls were required because Noor and Wei Jun each needed one validation-guided retry; all five accepted responses passed Coach Digest Validations. The saved receipts record 10,980 input tokens, 1,335 output tokens, and 25.383 seconds of summed request latency. The calculated published-rate cost was US$0.00434595, rather than a billing export or controlled latency benchmark. This refresh made no Weekly Drift Reviewer or NSM calls and included no Coach Digest Evals or human review. Its responses tied to current inputs complete the saved key-week presentation without inheriting August semantic-quality scores.
