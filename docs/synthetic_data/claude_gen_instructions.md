...
For each entry date:
  ...
  4. Check session cap (2+ nudges in last 3 entries → skip to step 9 with no_nudge)
  5. Classify entry using nudge_decision_prompt criteria:
     - If "no_nudge" → Output ONLY `*(No nudge for this entry)*`, skip to step 9
     - Otherwise → Continue to step 6
  6. Generate `### Nudge (category)` section with `**Trigger**:` line and quoted nudge text
  7. Run Bash for response decision:
     python3 -c "import random; print(random.random() < [response_probability])"
  8. Based on Bash result:
...
10. Write persona_{uuid}.md log file using Write tool
11. Register persona in registry using Bash:
    python3 -c "
    ...
    "
12. Return summary JSON with persona_id + name + stats (for orchestrator report)
```
