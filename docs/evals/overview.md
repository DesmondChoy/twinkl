...
| Drift Detection | 🟡 Partial | The empirical reference independently checks each declared core value for stored five-pass Judge consensus `-1` labels on two adjacent entries; other values do not offset the episode ([trajectory EDA](../drift/trajectory_eda.md)). The runtime target is rolling soft `P(-1)` evidence with uncertainty gating and weekly delivery; it is not implemented yet. The repo also contains an older weekly crash/rut/evolution router and six-detector review comparison; these are experimental implementation surfaces rather than the v1 contract. | Implement the soft-evidence v1 detector, calibrate it on label-derived candidates, add an unbiased held-out scripted set, and report event hit rate, precision, recall, F1, false-positive rate, and latency. Add active, recovered, mixed, and uncertain delivery semantics. Evolution/fade/rise taxonomies remain parked. |
