...
| Value Modeling | 🟡 In Progress | The 56-run / 120-config archive keeps `run_019`-`run_021` BalancedSoftmax as the active corrected-split frontier at median QWK **0.362**, `recall_-1` **31.3%**, minority recall **44.8%**, and calibration **0.713**. Qwen (`run_042`-`run_044`), two-stage (`run_045`-`run_047`), consensus-label (`run_048`-`run_050`), and recall-aware retention (`run_051`-`run_056`) remain diagnostic. The LLM context baseline improves QWK/minority recall with history but does not replace the MLP on `recall_-1` or hedging ([experiment index](../../logs/experiments/index.md)). | Repair the target/context contract (`twinkl-a30f`, `twinkl-j0ck`), build the matched hard-set (`twinkl-748`), prototype compact student context (`twinkl-749`), and compare candidates at the sustained-conflict decision layer before gated PEFT work (`twinkl-750`) |
| Drift Detection | 🟡 Partial | The empirical reference independently checks each declared core value for stored five-pass Judge consensus `-1` labels on two adjacent entries. `twinkl-wq9p` now provides a rerunnable 52-episode reference table, adjacent-window decisions, threshold search, event matching, frozen-split comparison, a locked 10-episode designed holdout, and a procedurally metadata-blinded Codex audit ([report](../../logs/experiments/reports/experiment_review_2026-07-10_twinkl_wq9p.md)). The MLP arms reached only 10–20% designed-holdout recall; the LLM arms reached 100% there but 0/5 on the consensus-derived frozen episodes. The audit qualified 1/5 frozen cases, 10/10 designed positives, and 0/10 controls, so no scorer is promotion-ready. | Repair the student-visible drift target and create an untouched promotion surface in `twinkl-v8pb`; the Codex audit is diagnostic AI evidence, not human ground truth. Production `P(-1)` persistence, the calibrated runtime detector, and active/recovered/mixed/uncertain Coach delivery remain blocked in `twinkl-a2w`. Evolution/fade/rise taxonomies remain parked. |
