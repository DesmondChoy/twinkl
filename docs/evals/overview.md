...
| Coach Digest Explanation Quality | 🟡 Partial | 1 594 / 1 651 persisted labels have rationales, the annotation tool can display or compare them, and Coach Digest Tier 1 response checks are implemented in [`src/coach/weekly_digest.py`](../../src/coach/weekly_digest.py) | Batch pass-rate reporting for Coach Digest responses, rationale-specific Tier 1 checks in `src/judge/`, and deeper rationale-review LLM and human-calibration work |
