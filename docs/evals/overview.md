...
| Drift Detection | 🟡 Partial | The empirical reference independently checks each declared core value for stored five-pass Judge consensus `-1` labels on two adjacent entries. `twinkl-wq9p` now provides a rerunnable 52-episode reference table, adjacent-window decisions, threshold search, event matching, frozen-split comparison, and a locked 10-episode designed holdout ([report](../../logs/experiments/reports/experiment_review_2026-07-10_twinkl_wq9p.md)). The MLP arms reached only 10–20% designed-holdout recall; the LLM arms reached 100% there but 0/5 on the consensus-derived frozen episodes, so no scorer is promotion-ready. | Human-review the frozen-versus-designed disagreement before selecting a scorer or narrowing the claim. Production `P(-1)` persistence, the calibrated runtime detector, and active/recovered/mixed/uncertain Coach delivery remain in `twinkl-a2w`. Evolution/fade/rise taxonomies remain parked. |
