...
| Drift Detection | 🟡 Partial | The empirical reference is two consecutive consensus `-1` labels on a declared core/high-weight value ([trajectory EDA](../drift/trajectory_eda.md)). The runtime target is rolling soft `P(-1)` evidence with uncertainty gating and weekly delivery. The repo also contains an older weekly crash/rut/evolution router and six-detector review comparison; these are experimental implementation surfaces rather than the v1 contract. | Implement the soft-evidence v1 detector, calibrate it on label-derived candidates, add an unbiased held-out scripted set, and report event hit rate, precision, recall, F1, false-positive rate, and latency. Evolution/fade/rise taxonomies remain parked. |
