...
*Implementation evidence through 16 September 2026; North Star Moment evaluation
uses the 7 September targeted comparison. The linked specifications and reports
record each component's scope and evidence. Capstone closeout decisions are
recorded through 19 September 2026.*
...
| **Coach Digest** | 🧪 Experimental | The Coach Digest runs after each Weekly Drift Detection result, including No Active Drift. It supplies the current state, a prior closed-week comparison, separate prior and current evidence, and cited Journal Entries to a prompt. It can describe that an active pattern did not continue only when a Not Conflict decision supports that deterministic change. It cannot treat Not Conflict as proof of improvement. If it cannot return a valid response, the Weekly Drift Detection result remains available. OpenAI response generation uses `gpt-5.6-luna` with reasoning effort `none`. The [August evaluation manifest](../logs/experiments/reports/coach_digest_sample_20260824/judge_sample_manifest.json) preserves five accepted responses for the previous Persona roster and Weekly Drift inputs. All five passed all Coach Digest Validations. The [September refresh](../logs/experiments/reports/demo_v4_run1_20260907/report.md) and [completion](../logs/experiments/reports/demo_coach_all_weeks_20260908/report.md) preserve the earlier roster's Luna-none generation provenance. The [Persona replacement run](../logs/experiments/reports/demo_persona_replacement_20260908/report.md) adds ten validated responses for Lukas and Meera and retains 17 compatible responses for Nisha, Noor, and Wei Jun. The [9 September voice refresh](../logs/experiments/reports/coach_voice_refresh_20260909/report.md) regenerates all 27 current replay weeks with prompt `4.4` from the same weekly inputs, preserving the earlier provider receipts. The responses pass Coach Digest Validations, including new checks for recap openings and clinical finding language, and retain matching input hashes. The five saved Personas also support a paired qualitative comparison: the default Coach Digest is generated without selected North Star Moment context; the toggle shows a separate response generated with that context, including its own question and the exact source panel. Inspect exposes both prompts and associated responses. Onboarding retains its original narrative and question with an optional independently validated passage. No new Coach Digest Evals or human validation was performed. The historical August Coach Digest Evals scored mean correctness `4.80`, specificity `5.00`, non-prescriptive tone `5.00`, and tension honesty `4.60`; all reflective questions passed, with no failed verdicts or review flags. These scores are same-model AI review, not human validation. Coach Digest Evals can select OpenAI or Gemini independently from the generator and record both model identities. The deterministic Drift/control study selects the 42 known development Drifts and 42 matched controls, supports safe resume, and reports pass rates from Coach Digest Validations and means from Coach Digest Evals by group. The independent-provider and 42-Drift/42-control evaluation workstream is Done under the accepted tooling scope (`twinkl-sosp`). No paid cross-provider or Drift/control result is committed or required for capstone completion. Human calibration and the external user pilot are closed without execution; a fresh final test and deployment approval are outside capstone scope. |
...
| **Experience and Inspect React App** | 🚧 In Progress | Shared contracts, five deterministic week-by-week Persona replays, manual Journal Entries, displayed nudges with reply and skip actions, Journal Entry removal, Simulated time, explicit closed-week review, affected-week recomputation, safe retry, live Weekly Drift Reviewer integration, Drift, required Coach Digest runs, and linked Inspect events are implemented. Manual Journal Entry cards show the newest entry first, while Weekly Drift Detection and Inspect keep chronological order. Monday-through-Sunday weeks become eligible only after Sunday closes; saving a Journal Entry never reviews its open week. Persona selection, staggered selected-week Journal Entries and nudge responses at 500 ms intervals, explicit Weekly Drift Detection result review, direct selection of every week before review, next-week navigation without a review gate, restart, named jumps to key weeks, reduced-motion behavior, and no-future-data projection are wired into the shared React session. Scenario requests bypass the browser cache and retain catalogued SHA-256 verification. The saved replay uses Weekly Drift prompt v4 Run 1 and completed full-history North Star Moment outcomes across 27 weeks for Nisha, Noor, Lukas, Wei Jun, and Meera. The chooser displays their weekly states in five selectable rows, an informational sidebar, and one action to start or continue the selected replay; its header identifies the demo without an onboarding Inspect hint. Nisha demonstrates a Drift emerging, Noor six weeks without Active Drift, Lukas one Drift continuing across four weeks, Wei Jun Insufficient Evidence, and Meera Active Drift for Self-Direction while Tradition has No Active Drift. Meera's chooser row labels each Core Value separately. Dialogs retain keyboard focus and prevent background interaction. The Inspect switch follows the visible replay panel: Journal Entries opens Recorded work for saved entry and nudge processing; the weekly result opens its focused explanation. Inspect preserves that context across reloads. Inspect decision opens the current weekly explanation, and weekly Inspect event filters sit beside their affected lists with matching counts. Manual Write navigation appears only when its composer exists. The top-right Choose another Persona action returns from saved replay Experience or Inspect to the five-Persona chooser while preserving progress; the Twinkl wordmark returns to the landing page. Sessions holding a retired Persona return safely to the current chooser. Coach Digest responses require matching current inputs, and Inspect treats complete and reused Coach Digest events as available responses. A first-use notice gates manual writing, and Delete session removes matching browser and Python state only after confirmed Python deletion. The core release quality gate is complete. Longitudinal Core Value history is Done (`twinkl-rklc.30`): the Reviewed week selector restores each stored result, Coach Digest response, and linked Inspect events from retained traces. Coach Digest feedback and final professor walkthrough evidence remain open. Optional live rerun is Not done and closed outside capstone scope. Desktop and laptop browsers are the primary verification target; the existing narrow-screen flow remains supported. [Design](demo/experience_inspect_app.md) |
...
| **North Star Moment** | 🧪 Targeted AI evaluation integrated in saved replay | Live application code uses full-history `gpt-6-luna` at `low` reasoning in onboarding; all five saved Persona replays retain their `gpt-5.6-luna` receipts, including No Active Drift weeks. Accepted current-week supportive actions receive specific encouragement; older examples receive reminder wording; Active Drift uses pre-onset writing. No Active Drift alone does not establish alignment. Exact quotation, owner, chronology, response availability, and bounded failure/retry checks apply to both paths. The completed North Star Moment update using saved Weekly Drift v4 Run 1 outputs reassessed both methods on 38 affected cases and retained observations for 463 unchanged cases, preserving the original 105 Personas and 81/24 development/final partition. Full eligible history still achieved higher Card precision and Opportunity recall than Nomic top-three retrieval in both partitions. This targeted update retains evidence from the original v2 repeat-1 comparison; it is AI assessment of synthetic histories, without human review; human calibration and the user pilot are outside capstone scope. Saved replay preserves the completed full-history selections and no-card outcomes for its five Personas. Live NSM uses a separately authorized US$1 allowance shared across sessions and restarts. Both routes place a validated selection within a valid Coach Digest after its reflective question. Saved replay compares separately generated responses with and without that selected source context, and Inspect shows both prompts and responses. Onboarding preserves its original response. Persona replay identifies the passage and links its exact Inspect event. [Specification](north_star/north_star_moment.md), [methodology](north_star/nsm_experiment_methodology.md), [targeted results](../logs/experiments/reports/north_star_v4_run1_20260907/report.md) |
...
Ordinary Coach Digest inputs retain the complete displayed text of each selected
...
### Capstone closeout decisions

| Workstream | Final status | Accepted scope and evidence |
|---|---|---|
| Longitudinal Core Value history (`twinkl-rklc.30`) | Done | Manual Experience provides chronological selection of reviewed weeks, Core Value states, cited evidence, Coach Digest responses, and linked Inspect events. Retained traces support session restore and replacement of recomputed weeks. A separate weekly-summary collection is outside the accepted implementation. |
| Independent-provider Coach evaluation and 42-Drift/42-control study (`twinkl-sosp`) | Done | Provider selection, deterministic targets, resume, provenance, and comparison reporting satisfy the accepted capstone scope. No paid independent-provider study result is committed; further execution is not required for completion. |
...
* **Target users / addressable market**
    ...
    * Use five curated personas for the capstone menu, with one recommended walkthrough. Together they cover three Schwartz Core Values, all three current Drift states, continuing Drift across weeks, contrasting Core Value states, and Historical Drift Records that end.
...
3. **Reasoning + action:** The required user-facing path reviews closed Monday-through-Sunday calendar weeks. The first partial week becomes eligible after its first Sunday; saving a Journal Entry does not review the open week. The completed **[VIF Critic (Offline)](vif/01_concepts_and_roadmap.md)** remains an offline research component:
   ...
   * **Weekly Drift Detection:** At the end of each week, the internal Weekly Drift Reviewer reviews Journal Entries without VIF Critic Predictions. Its current model contract is `gpt-6-luna` with reasoning effort `low`; prompt version `4.0` supplies the selected Core Values' existing `definition` and `core_motivation` fields as Schwartz-based definitions plus project-specific elaborations. The internal Drift Detector then applies the rule that two consecutive Conflicts for the same Core Value form Drift. The Drift Detector recomputes all stored decisions. It stores Active Drift, No Active Drift, or Insufficient Evidence per Core Value and keeps Historical Drift Records separately. It fails closed to Abstain on invalid or refused responses. The complete development analysis contains 42 Drifts across 36 Drift trajectories in 292 resolved cases. With prompt version `2.0`, the fixed Luna-low setup had median Drift recall of `0.548`, 4 false Drift alerts, and `0.637` coverage. These are AI-reviewed synthetic development results for that historical prompt. The capstone has no fresh final test or deployment approval. The crash/rut/evolution router remains only for historical compatibility.
   ...
   * **[North Star Moment](north_star/north_star_moment.md):** An eligible weekly result with a valid Coach Digest can include one exact quotation of an action supporting a confirmed Core Value after its displayed narrative and reflective question. The five saved Personas can switch between complete Coach Digest responses generated with and without selected source context; Inspect preserves both prompts and responses. Onboarding keeps its original narrative and question. Active Drift uses pre-onset writing; No Active Drift prefers current-week encouragement, then a historical reminder; Insufficient Evidence produces no card. Full-history Luna-low review and deterministic source checks apply to saved replay and the manual Experience. Saved selections and no-card outcomes are available for all five Personas; live review uses its pinned, separately authorized budget.
...
- The Weekly Drift Reviewer model contract is fixed at `gpt-6-luna` with
...
- The three historical GPT-5.6 Luna low development Runs supported the original
  capstone POC contract: each achieved Drift recall above `0.50`,
  with 5, 4, and 4 false Drift alerts across 256 non-Drift Core Value
  trajectories. The user accepted the version `3.0` prompt cleanup without a
  development rerun. The later comparison of versions `3.0` and `4.0` adds
  both definition and motivation context and is documented in the
  [Weekly Drift evaluation contract](evals/drift_detection_eval.md).
...
- `twinkl-q6pt` and `twinkl-gv3x` compare `gpt-5.6-luna` with `gpt-6-luna`
  on the same prompt-version-`4.0` development requests at every reasoning
  effort from `none` through `xhigh`. No paired Drift-recall difference is
  established. GPT-6 Luna has higher coverage at every effort and lower
  calculated token cost. At `low`, median Drift recall is `0.381` for
  GPT-5.6 Luna and `0.405` for GPT-6 Luna, with 3 and 2 median false Drift
  alerts. The live Weekly Drift Reviewer now uses GPT-6 Luna at `low` based on
  lower calculated cost and higher coverage. The paired Drift-recall
  difference is 0.000 [−0.095, +0.089], so a recall improvement is unproven.
...
- `weekly_digest_coach.yaml` — Coach Digest prompt `4.7`, with cited writing,
  concrete conversational openings, ordinary expressions of uncertainty, and
  one short reflective question; generation checks reject recap openings
  ...
  an explicit evidence-limit statement. Prompt `4.7` responses require an exact weekly
  quotation in `weekly_mirror`, exact source matches for every quotation, three
  nonempty fields, and one generated question across the complete response.
  Exact source quotations are excluded from generated-question counts and
  unsupported-state-claim checks. The prompt preserves who did what, the means
  of interaction, and uncertainty about motives. Explicit retries use the
  previous matching validation errors; original attempts remain in Inspect.
  Responses have no minimum length and a 180-word maximum. Mechanical checks
  do not establish semantic accuracy. Saved responses retain their original
  validation rules. The [onboarding QA fix verification](../logs/experiments/reports/onboarding_qa_fixes_20260916/report.md)
  records regression coverage and three source-reviewed development responses.
...
[`src/north_star/assessment.py`](../src/north_star/assessment.py) and
[`src/north_star/runtime.py`](../src/north_star/runtime.py). The experiment's
independent AI evaluation uses the contract in the
[North Star Moment methodology](north_star/nsm_experiment_methodology.md).
...
* Desktop and laptop browsers are the primary assessment and verification
  target for React Experience and Inspect. The responsive layout also supports
  narrow-screen phones. Native mobile packaging is outside the capstone scope.
...
* **North Star Moment** 🧪: Quote an eligible supportive action from the user's
  own writing beneath a weekly reflection, with source checks and a separate
  AI review. See the [specification](north_star/north_star_moment.md) for card
  eligibility and the distinction between saved and live review.
* **Conversational introspection agent** ✅: The displayed nudge interaction is
  ...
  Decisions. Response rate, continued journaling, and perceived relevance
  have no collected external-user evidence. The user pilot is closed outside
  capstone scope.
...
7. Evaluation plan: collect optional Coach Digest perceived-accuracy feedback
   without changing the fixed Profile. The external user pilot is closed
   outside capstone scope; broader user measures are not collected.
...
the Coach Digest while keeping unmeasured user-perceived usefulness separate
from AI-reviewed synthetic development evidence. Human calibration and the
external user pilot are closed outside capstone scope.
...
**Validation approach:** Synthetic stress tests cover technical correctness.
The proposed 5–10-person, 1–2-week study of perceived accuracy is not conducted
and is closed outside capstone scope.
...
| [nudge_design_rationale.md](pipeline/nudge_design_rationale.md) | Displayed nudge product design rationale and optional external-study measures |
| **VIF** | |
...
| [twinkl-q6pt Luna model comparison](../logs/experiments/reports/experiment_review_2026-09-23_twinkl_q6pt_luna_model.md) | Prompt-v4 GPT-5.6 and GPT-6 Luna results at `none` and `low` |
| [twinkl-gv3x Luna higher-reasoning model comparison](../logs/experiments/reports/experiment_review_2026-09-23_twinkl_gv3x_luna_higher.md) | Prompt-v4 GPT-5.6 and GPT-6 Luna results at `medium`, `high`, and `xhigh` |
| **Other** | |
