...
3. **Reasoning + action:** The required user-facing path reviews closed Monday-through-Sunday calendar weeks. The first partial week becomes eligible after its first Sunday; saving a Journal Entry does not review the open week. The completed **[VIF Critic (Offline)](vif/01_concepts_and_roadmap.md)** remains an offline research component:
   ...
   * **[North Star Moment](north_star/north_star_moment.md):** An eligible weekly result can show one exact quotation of an action supporting a confirmed Core Value beneath the Coach Digest. Active Drift uses pre-onset writing; No Active Drift prefers current-week encouragement, then a historical reminder; Insufficient Evidence produces no card. Full-history Luna-low review and deterministic source checks apply to saved replay and the manual Experience. Saved selections and no-card outcomes are available for all five Personas; live review requires a valid integration budget.
