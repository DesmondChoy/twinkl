...
> current user-facing Drift path does not run the VIF Critic or consume VIF
> Critic Predictions.
...
- An Abstain or failed Weekly Drift Reviewer Decision produces Insufficient
  Evidence when it blocks recent Conflict evidence. It produces no Drift claim;
...
- Any future fresh final test should reuse the Luna-low response schema,
  ...
  acceptance criteria.
- The ternary ten-value VIF Critic output remains available for offline
  reproduction. Weekly Drift Reviewer Decisions drive the current user-facing
  path. VIF-Critic-proposed candidates and a review-and-retrain loop are not
...
These ideas are outside the time-boxed capstone. No further VIF Critic work
...
| **System & safety** | Latency from entry → feedback, LLM failure rates, data-notice acknowledgement, confirmed session deletion, provider boundaries, and qualitative review of the non-therapy message. |
