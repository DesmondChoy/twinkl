...
| **North Star Moment** | ❌ Not Started | Scope adopted on 2026-09-05 for both frontend paths: all five saved Persona demos and onboarding from scratch. After an eligible closed-week result, show at most one exact quotation of an earlier supportive action, with correct omission when none qualifies. Demo results are prepared offline; onboarding requires live retrieval and AI review, bounded retries, and source-change invalidation. User-facing copy must not expose internal Drift states or describe the user as drifting. Implementation, evaluation, and browser QC remain outstanding. [Specification](future_work/north_star_moment.md) |
