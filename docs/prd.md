...
| **Drift Detector** | 🧪 Experimental | Drift is two consecutive Conflicts for the same Core Value. The former consensus-derived frozen benchmark is [retired historical evidence](archive/evals/retired_wq9p_drift_benchmark_2026-07-11.md). [`twinkl-752.4`](../logs/experiments/reports/experiment_review_2026-07-13_twinkl_752_4_legacy_drift_review.md) reviewed all 52 legacy-discoverable candidates plus 52 matched controls under the displayed-behavior rule. The development reference now contains 27 Drift episodes across 23 resolved trajectories; four more episodes are quarantined as retired-test audit evidence. Four trajectories remain uncertain. This is selection-biased AI-reviewed development evidence, not prevalence or a fresh final test. `twinkl-752.5` must compare MLP-triggered early review with weekly-only and matched-budget model-free schedules before the user decides the architecture in `twinkl-752.2`. No setup has deployment approval, and the weekly crash/rut/evolution prototype is still not the selected Drift Detector. |
...
3. **Reasoning + action:** A two-stage evaluative layer powered by the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)**:
   ...
   * **Drift Detector:** Reads VIF Critic predictions over time. Drift is two consecutive Conflicts for the same Core Value; other values do not cancel it. `twinkl-752.4` expanded the development reference to 27 episodes across 23 resolved trajectories by reviewing every candidate discoverable from either legacy label source plus matched controls. Four additional retired-test audit episodes stay outside the primary rates, four trajectories remain uncertain, and Drifts missed by both legacy sources may remain undiscovered because controls sample only part of that pool. These labels are valid development references even when the MLP saw the same Journal Entries during training, but scheduler scores on those entries are in-sample. A fresh final test is still required. The intended runtime may use soft `P(-1)` evidence under uncertainty gating, but no MLP, LLM, or scheduling architecture is selected. The existing crash/rut/evolution router is an experimental predecessor, not the selected v1 contract.
...
| [twinkl-752.4 full review](../logs/experiments/reports/experiment_review_2026-07-13_twinkl_752_4_legacy_drift_review.md) | Expanded development reference and scheduler-study handoff |
| **Other** | |
