...
| **VIF Critic Training** | 🧪 Experimental | Training stack complete with ordinal MLP heads, BNN baseline, and configurable sentence encoders (`nomic` active default; MiniLM/mpnet ablations). `run_019`-`run_021` remains the historical corrected-split reference. The paired `run_057`-`run_062` target experiment found that repaired Security supervision raises median test Security QWK by about 0.17 under both repaired and historical scoring lenses without regressing aggregate QWK. The controlled `run_063`-`run_068` vote-distribution experiment reduced hedging and improved Hedonism/minority recall, but weakened `recall_-1`, Security, Stimulation, and raw probability scores; soft BalancedSoftmax is therefore diagnostic rather than the new default. Future `window_size: 1` work should use the repaired Security regime, while keeping cross-regime scores separate and treating richer history as unresolved representation work. |
