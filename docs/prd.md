...
| **North Star Moment** | 🧪 Fresh AI evaluation complete; export pending | Application code supports full-history `gpt-5.6-luna` at `low` reasoning in onboarding and all five saved Persona replays, including No Active Drift weeks. Accepted current-week supportive actions receive specific encouragement; older examples receive reminder wording; Active Drift uses pre-onset writing. No Active Drift alone does not establish alignment. Exact quotation, owner, chronology, response availability, and bounded failure/retry checks apply to both paths. The fresh 501-week comparison found higher Card precision and Opportunity recall for full history than Nomic top-three retrieval in both development and qualified final evaluation. This is AI assessment of synthetic histories; human review remains deferred. Saved cards require fresh export, and live NSM fails closed without the removed integration budget. [Specification](north_star/north_star_moment.md), [methodology and results](north_star/nsm_experiment_methodology.md#completed-results) |
