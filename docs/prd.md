...
| **Judge Labeling (VIF)** | ✅ Complete | 1,651 entries labeled across 204 personas; two-phase pipeline (Python wrangling + parallel subagents); consolidated to `judge_labels.parquet` with rationales. The completed `twinkl-747` audit established a hard-dimension target-contract risk, especially for `security`, but its legacy prompt arms did not exactly match the active Critic state and did not create a repaired target. |
| **VIF Critic Training** | 🧪 Experimental | Training stack complete with ordinal MLP heads, BNN baseline, and configurable sentence encoders (`nomic` active default; MiniLM/mpnet ablations). The experiment archive contains 56 run IDs / 120 persisted run configs; `run_019`-`run_021` BalancedSoftmax remains the corrected-split frontier reference point. Recall-aware candidate retention (`run_051`-`run_056`) is experiment hygiene rather than the default selector. The historical reachability audit and frozen-holdout LLM context baseline support repairing the target/context contract before another broad model sweep, but no training-ready or evaluation-ready Security target exists yet. |
