...
3. **Reasoning + action:** Two connected evaluation paths combine the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)** with independent weekly review:
   ...
   * **Drift Detector:** Receives Weekly Drift Reviewer decisions made without VIF Critic input. Drift is two consecutive Conflicts for the same Core Value; other values do not cancel it. The complete development analysis contains 42 Drifts across 36 Drift trajectories in 292 resolved cases. The `twinkl-752.5` study used 33 of those Drifts in 106 cases; raw VIF Critic input did not reliably improve weekly review, and VIF-Critic-triggered early-plus-weekly review changed timing without adding Drift hits. Its offline trigger placements beat random placement on that smaller development input, but the scheduling timing effect disappeared outside training-seen Journal Entries. On the complete data, `gpt-5.4-mini` at reasoning effort `none` had median Drift recall of `0.167` with 5 false Drift alerts, while `gpt-5.6-luna` at reasoning effort `none` had `0.476` with 13. The current development Weekly Drift Reviewer is `gpt-5.6-luna` at reasoning effort `low`: median Drift recall is `0.548`, false Drift alerts are 4, and coverage is `0.637`. The approved metric hierarchy treats coverage as diagnostic rather than gating, and the study stops before `medium`. A fresh final test is still required. The optional VIF Critic candidate-confirmation path is outside the remaining capstone scope; revisiting it requires a new scope decision and fresh evaluation. The existing crash/rut/evolution router is an experimental predecessor, not the selected v1 contract.
...
- The ternary ten-value output remains. The adopted staged architecture keeps
  ...
  candidates are not part of the remaining capstone scope.
...
| [demo/weekly_drift_review_app.md](demo/weekly_drift_review_app.md) | Read-only comparison of frozen Weekly Drift Reviewer development Runs |
