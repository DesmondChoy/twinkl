...
| **Drift Detection Engine** | 🧪 Experimental | Drift v1 is a sustained conflict episode: two consecutive consensus `-1` reference labels on a declared core/high-weight value. The runtime target is rolling soft `P(-1)` evidence under uncertainty gating, delivered in the weekly Coach digest. The repository still contains a weekly crash/rut/evolution prototype router and a six-detector comparison surface; neither is the selected v1 detector. The consensus EDA supplies tuning candidates, while soft-evidence implementation, calibration, and unbiased held-out evaluation remain open. |
...
3. **Reasoning + action:** A two-stage evaluative layer powered by the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)**:
   ...
   * **Drift detector:** Reads Critic outputs over time. The v1 benchmark requires two consecutive consensus `-1` labels on a declared core/high-weight value; the runtime design estimates the same sustained conflict from rolling soft `P(-1)` evidence under uncertainty gating. The existing crash/rut/evolution router is an experimental implementation surface, not the selected v1 contract.
...
| 3 | **Drift detection** | Confirm the runtime detects sustained conflict with a declared core value | Use held-out sustained-conflict episodes defined by two consecutive consensus `-1` labels, plus scripted episodes for unbiased evaluation. Measure event hit rate, precision, recall, false-positive rate, and alert latency. Single-entry dips, fade/rise taxonomies, and value evolution remain outside v1. | Ground truth: two consecutive Benevolence conflicts form one episode. The soft-evidence detector flags the episode for the weekly digest. Target: ≥8/10 held-out episodes detected. |
