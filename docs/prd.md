...
| **Experience and Inspect React Demo** | 🚧 In Progress | Shared contracts, five deterministic persona replays, manual Journal Entries, displayed nudges with reply and skip actions, safe retry, and linked nudge events in Inspect are implemented. The remaining work connects persona replay, weekly review, Drift, and the Weekly Digest. Narrow-screen phones remain the primary verification target. [Design](demo/experience_inspect_app.md) |
