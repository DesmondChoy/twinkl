...
| **Drift Detection Engine** | 🧪 Experimental | Drift v1 is a per-value sustained conflict episode: the same declared core value receives a stored five-pass Judge consensus `-1` label on two adjacent entries. Other value dimensions do not offset that episode. The runtime target is rolling soft `P(-1)` evidence under uncertainty gating, delivered in the weekly Coach digest. The repository still contains a weekly crash/rut/evolution prototype router and a six-detector comparison surface; neither is the selected v1 detector. The trajectory EDA supplies tuning candidates, while soft-evidence implementation, calibration, and unbiased held-out evaluation remain open. |
...
| 3 | **Drift detection** | Confirm the runtime detects sustained conflict with a declared core value | Use held-out per-value episodes defined by stored five-pass Judge consensus `-1` labels on two adjacent entries for the same declared core value, plus scripted episodes for unbiased evaluation. Measure event hit rate, precision, recall, false-positive rate, and alert latency. Single-entry dips, fade/rise taxonomies, and value evolution remain outside v1. | Reference event: two adjacent Benevolence conflicts form one episode even if another core value remains aligned. The soft-evidence detector flags the episode for the weekly digest. Target: ≥8/10 held-out episodes detected. |
