...
| **North Star Moment** | 🚧 Blocked at feasibility gate | Scope adopted on 2026-09-05 for all five saved Persona demos and onboarding from scratch: at most one exact quotation of an earlier supportive action after an eligible closed-week result, with correct omission when none qualifies. The small separate benchmark and US$20 paid ceiling were approved before dependent work. Phase 0A passed at k=3 (21/22 positive-label histories), but Phase 0B failed: 12/19 selections accepted by independent AI reference review, 5/9 correct omissions, and 2/29 contract-invalid OpenAI attempts. The specification's stop rule retains existing Coach Digest behavior. Offline feasibility code and evidence are implemented; runtime integration, React cards, NSM Inspect, final evaluation, and browser QC are blocked. A [development revision](../logs/experiments/reports/north_star_phase0b_revision_20260905/README.md) passes 300 NSM tests but also fails the semantic gate: 11/19 selections accepted and 7/9 correct omissions, with zero failures across 40 new provider calls. Its stricter reference-consistency rule also scores the original selections 11/19; a raw precision decline is not established model regression. [Specification](north_star/north_star_moment.md), [original measured results](../logs/experiments/reports/north_star_phase0b_20260905/README.md) |
