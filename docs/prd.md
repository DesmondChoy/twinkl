...
| **VIF Critic Training** | 🧪 Experimental | Training stack complete with ordinal MLP heads, BNN baseline, and configurable sentence encoders (`nomic` active default; MiniLM/mpnet ablations). The experiment archive contains 56 run IDs / 120 persisted run configs; `run_019`-`run_021` BalancedSoftmax remains the corrected-split frontier reference point. Recall-aware candidate retention (`run_051`-`run_056`) is experiment hygiene rather than the default selector. The `twinkl-747` reachability audit and frozen-holdout LLM context baseline both support repairing the target/context contract before another broad model sweep. |
...
| **Drift Detection Engine** | 🧪 Experimental | Drift v1 is a per-value sustained conflict episode: the same declared core value receives a stored five-pass Judge consensus `-1` label on two adjacent entries. The `twinkl-wq9p` benchmark now materializes 52 reference episodes, calibrates one two-entry `P(-1)` plus uncertainty rule, and evaluates MLP and LLM arms on the frozen split and a locked 10-episode designed holdout. No scorer is promotion-ready: `run_020` detected 1/10 designed episodes and consensus-trained MLPs detected 2/10, while both LLM arms detected 10/10 designed episodes but 0/5 consensus-derived frozen episodes. A procedurally metadata-blinded Codex audit then qualified 1/5 frozen cases, 10/10 designed positives, and 0/10 controls; it is AI diagnostic evidence, not human ground truth. The frozen reference is not a stable student-visible promotion surface, so `twinkl-v8pb` must repair the label/target contract and establish an untouched promotion surface before trigger wiring. The weekly crash/rut/evolution prototype is still not the selected v1 detector. |
...
3. **Reasoning + action:** A two-stage evaluative layer powered by the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)**:
   * **Critic (VIF):** A numeric, uncertainty-aware engine that computes per-value-dimension alignment estimates from the current student-visible state. It uses [LLM-as-Judge for reward modeling](vif/03_model_training.md) and [MC Dropout for epistemic uncertainty](vif/04_uncertainty_logic.md). The current config uses one journal session plus the normalized 10-dimensional value profile; larger legal-history windows remain an experiment.
   * **Drift detector:** Reads Critic outputs over time. The v1 benchmark independently checks each declared core value for stored five-pass Judge consensus `-1` labels on two adjacent entries; labels on other values do not cancel the episode. The `twinkl-wq9p` benchmark implements and calibrates the decision-level evaluation harness, but its MLP/LLM cross-set disagreement does not support promoting a production scorer yet. The intended runtime still estimates sustained conflict from rolling soft `P(-1)` evidence under uncertainty gating. The existing crash/rut/evolution router is an experimental implementation surface, not the selected v1 contract.
...
| 3 | **Drift detection** | Confirm the runtime detects sustained conflict with a declared core value | Use held-out per-value episodes defined by stored five-pass Judge consensus `-1` labels on two adjacent entries for the same declared core value, plus an isolated author-designed capability set. Measure event hit rate, precision, recall, false-positive rate, and alert latency. The designed cases have a procedurally metadata-blinded Codex audit but remain author-designed rather than human ground truth; `twinkl-v8pb` must establish a repaired student-visible target and untouched promotion surface before a scorer can be promoted. Single-entry dips, fade/rise taxonomies, and value evolution remain outside v1. | Reference event: two adjacent Benevolence conflicts form one episode even if another core value remains aligned. The soft-evidence detector flags the episode for the weekly digest. Capability check: report recall on the 10 author-designed positives and false positives on the 10 matched controls. This set cannot promote a scorer; promotion requires the repaired target and untouched evaluation surface from `twinkl-v8pb`. |
...
| [evals/drift_detection_eval.md](evals/drift_detection_eval.md) | Sustained-conflict reference, runtime target, and benchmark protocol |
| **Other** | |
