...
| **Experience and Inspect React Demo** | 🚧 In Progress | Shared contracts, five deterministic persona replays, manual Journal Entries, displayed nudges with reply and skip actions, safe retry, live Weekly Drift Reviewer integration, Drift, cited Weekly Digests, and linked Inspect events are implemented. Persona replay controls and release hardening remain. Narrow-screen phones remain the primary verification target. A nudge reply added after submission does not yet trigger weekly recomputation. [Design](demo/experience_inspect_app.md) |
