...
| **Drift Detection Engine** | ⚠️ Partial | Intended to be the main weekly inference layer that detects value drift from journal-entry alignment patterns, classifies the response mode (`stable`, `rut`, `crash`, `high_uncertainty`, etc.), and passes both the mode and supporting reasons downstream to the weekly digest generator; formulas and design are specified, but the production engine is not yet implemented |
...
3. **Reasoning + action:** A two-stage evaluative layer powered by the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)**:
   ...
   * **Drift Detection Engine:** Sits downstream of Critic outputs and upstream of the weekly Coach. It is intended to be the main engine that detects weekly drift from journal-entry alignment patterns, decides the response mode (`stable`, `rut`, `crash`, `high_uncertainty`, etc.), and produces the structured reasons that are passed into the weekly digest generator.
   * **Coach:** Activated after the Drift Detection Engine classifies the week's mode and supporting rationale. Reads the user's full journal history via **full-context prompting** (at POC scale, all entries fit in the LLM context window) to surface thematic evidence, explain *why* the week's pattern matters, and offer reflective prompts. For positive patterns, provides occasional evidence-based acknowledgment without gamification. At production scale with longer histories, this would transition to retrieval-augmented generation (RAG). (See [System Architecture](vif/02_system_architecture.md)). For a concrete scenario, see [Worked Example: Sarah's Journey](vif/example.md). *(Entry processing ready; drift detection and digest generation remain partial — see [Implementation Status](#implementation-status).)*
