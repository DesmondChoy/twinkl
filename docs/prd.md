...
| **North Star Moment** | 🧪 Implementation retained; fresh evaluation pending | Application code supports full-history `gpt-5.6-luna` at `low` reasoning in onboarding and all five saved Persona replays, including No Active Drift weeks. Accepted current-week supportive actions receive specific encouragement; older examples receive reminder wording; Active Drift uses pre-onset writing. No Active Drift alone does not establish alignment. Exact quotation, owner, chronology, response availability, and bounded failure/retry checks apply to both paths. Previous NSM results, findings, and generated review records have been removed. Saved cards require fresh export, and live NSM fails closed without the removed integration budget. The fresh methodology is pending user review. [Specification](north_star/north_star_moment.md), [fresh experiment methodology](north_star/nsm_experiment_methodology.md) |
