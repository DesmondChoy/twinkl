...
| **Drift Detector** | 🧪 Experimental | Drift is two consecutive Conflicts for the same Core Value. The former consensus-derived frozen benchmark is [retired historical evidence](archive/evals/retired_wq9p_drift_benchmark_2026-07-11.md). [`twinkl-752.4`](../logs/experiments/reports/experiment_review_2026-07-13_twinkl_752_4_legacy_drift_review.md) reviewed all 52 legacy-discoverable candidates plus 52 matched controls under the displayed-behavior rule and found 31 episodes across 26 Drift trajectories. Three overlap the earlier five; adding the two prior episodes omitted by candidate mining produces the 33-episode / 28-Drift-trajectory known-development union. A [blind Opus follow-up](../logs/experiments/reports/experiment_review_2026-07-14_twinkl_752_5_opus_null_resolution.md) resolved the four remaining trajectories without adding Drift, so all 106 union trajectories are resolved. Four reviewed episodes came from the former final-test split and must be reported as a provenance subgroup, not excluded from development evidence. This is selection-biased AI-reviewed development evidence, not prevalence or a fresh final test. `twinkl-752.5` must compare weekly review without VIF Critic input, weekly review with raw VIF Critic input, and VIF-Critic-triggered early-plus-weekly review before the user decides the architecture in `twinkl-752.2`. A separate offline diagnostic compares the realized trigger placements with random placements at the same count without making Weekly Drift Reviewer calls. No setup has deployment approval, and the weekly crash/rut/evolution prototype is still not the selected Drift Detector. |
...
3. **Reasoning + action:** A two-stage evaluative layer powered by the **[Value Identity Function (VIF)](vif/01_concepts_and_roadmap.md)**:
   ...
   * **Drift Detector:** Reads VIF Critic predictions over time. Drift is two consecutive Conflicts for the same Core Value; other values do not cancel it. `twinkl-752.4` reviewed every candidate discoverable from either legacy label source plus matched controls and found 31 episodes across 26 Drift trajectories. Three overlap the earlier five; the two prior episodes missed by that cohort bring the known-development union to 33 episodes across 28 Drift trajectories. A blind Opus follow-up resolved the four remaining trajectories without adding Drift, so all 106 union trajectories are resolved. Four reviewed episodes came from the former final-test split; include them in the primary development analysis and report provenance subgroups separately. Other Drifts missed by both legacy sources may remain undiscovered because controls sample only part of that pool. These labels are valid development references even when the MLP saw the same Journal Entries during training, but MLP scores on those entries are in-sample. A fresh final test is still required. The intended runtime may use soft `P(-1)` evidence under uncertainty gating, but no MLP, LLM, or scheduling architecture is selected. The existing crash/rut/evolution router is an experimental predecessor, not the selected v1 contract.
...
| 3 | **Drift detection** | Confirm the Drift Detector finds Drift for a Core Value | Use the displayed-behavior target: each of two consecutive Journal Entries must clearly show Conflict against the same Core Value. The known-development union contains 33 episodes across 28 Drift trajectories; report the four former-final-test episodes as a provenance subgroup, not a separate evaluation set. `twinkl-752.5` compares weekly review without VIF Critic input, weekly review with raw VIF Critic input, and VIF-Critic-triggered early-plus-weekly review. Its offline trigger-placement diagnostic makes no Weekly Drift Reviewer calls. Optimize Drift recall, then choose a conservative false-alert and review-cost constraint. A fresh final test is still required. | Drift: two consecutive Journal Entries both visibly show Conflict against Benevolence. An uncertain VIF Critic abstains. `+1` on another value cannot cancel the Drift. |
