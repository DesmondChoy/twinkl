...
## Active VIF Critic State

The Security target must be justified by the exact state available to the
VIF Critic at inference time. For the active `window_size: 1` frontier, that
state is:

```text
embedding(concatenate_entry_text(initial_entry, nudge_text, response_text))
+ normalized 10-dimensional value profile
```

The synthetic-persona profile assigns equal mass to each valid Core Value and
falls back to a uniform vector if none match. With `window_size: 1`, there is no
time-gap feature and no earlier Journal Entry in the state.

The active state excludes:

- persona name, age, profession, culture, and biography;
- the date of the current Journal Entry and prior or future Journal Entries;
...
label may use the value profile to interpret a trade-off, but it must be
grounded in behavior, choices, or attitudes visible in the current Journal
Entry, including its displayed nudge and response when present.
...
of its three original prompt setups exactly represented the active VIF Critic
state. In particular, its legacy `student_visible` setup omitted the profile
while retaining persona demographics and date. Its `profile_only` setup
retained raw Core Value names plus the same unavailable metadata. Both used the historical
session format used by the LLM-Judge rather than the runtime serialization.

The 14-row target parquet introduced in commit
`cc114f49f6af992e312a7f61790fad269306820e` therefore did not establish an
active-state Security target. It has been removed from the active experiment
output path
and recorded in the
...
The training-ready review used the complete corpus of 1,651 Journal Entries.
Each Journal Entry
...
accepted all Journal Entries and wrote a non-destructive target variant.
...
- outputs under `logs/exports/twinkl_a30f_active_critic_state_full_v1/` and
...
repeated LLM judgments with recorded disagreement, not independent human
...
- the runtime-formatted current Journal Entry, including its displayed nudge
  and response when present;
...
LLM-Judge or human reviewer. Case coordinates and hashes remain outside the
review input and are used only to reconcile the returned result.
...
duplicate Journal Entry coordinates. Materialization independently re-renders
the canonical prompt from the hashed state input, so a self-consistently rehashed
...
The builder derives `new_label` only from the exact `active_critic_state` setup.
...
directory, refuses to overwrite an existing target parquet, and binds the resulting
summary to the source, manifest, results, and parquet hashes.

## Conservative context buckets

Legacy labels may appear in a target parquet only as diagnostic provenance.
...
- `changes_with_bio_or_history`: the exact-state result matches the legacy
  ...
  isolated because biography and earlier Journal Entries were added together.
...
frozen test set. Even a completed exact-state review therefore remains:

- diagnostic-only;
- unsuitable for training or target-repair fitting;
- unsuitable as an old-label or repaired-label evaluation set; and
- unable to support deployment approval or a production-readiness claim.
