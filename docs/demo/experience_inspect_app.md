...
The expanded saved-generation and browser validation status is recorded in the
[integration record](../../logs/experiments/reports/north_star_integration_20260906/README.md).
The original [development protocol](../north_star/luna_evaluation_20260905.md)
and scores are unchanged. Reserved final evaluation and capstone report result
incorporation remain outstanding.
...
fixture through `frontend/onboarding/src/demoContracts.ts`. The canonical fixture retains the original event examples; focused tests and
saved bundles additionally exercise `north_star_reviewed` and
`nudge_response_recorded`. NSM completion updates its pending event, so clients
