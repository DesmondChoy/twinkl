...
Previous North Star Moment experiment results, findings, saved records, and
browser evidence have been removed. Saved replay has no NSM cards until a
fresh export; live NSM fails closed without the removed integration budget.
The [fresh experiment methodology](../north_star/nsm_experiment_methodology.md)
records the reset and is pending user review.
...
`north_star_reviewed` and `nudge_response_recorded`. Saved NSM records were
removed during the reset. NSM completion updates its pending event, so clients
