...
| **Weekly Drift Reviewer** | The internal LLM in Weekly Drift Detection that reviews Journal Entries for Conflict. The current model contract is `gpt-6-luna` with reasoning effort `low`, without VIF Critic predictions. Historical experiments used other setups. It is not the VIF Critic. | weekly verifier, LLM arm, system |
...
| **North Star Moment** | At most one exact quotation of the user's supportive action within a valid Coach Digest after an eligible closed-week result, after its narrative and reflective question. Active Drift uses writing from before onset; No Active Drift prefers current-week encouragement, then a historical reminder. Insufficient Evidence produces no card. Sources are eligible Journal Entries or user nudge responses with independent availability evidence. The five-Persona demo can compare complete Coach Digest responses generated with and without the selected source context, showing the quotation as added Coach Digest input above the with-context response; onboarding preserves its original narrative and question. The feature name is used in product documentation, Inspect, and discreet Persona replay attribution. Personal Experience headings are “A moment in your own words” or “A past moment in your own words,” without the feature name. | proof of recovery, alignment score, best or strongest example |
| **Coach Digest Validations** | The code checks applied when Twinkl creates a Coach Digest response. The checks cover grounded quotes, score jargon, raw Schwartz value leakage, current-state claims, response length, and, from prompt 4.4, recap openings and clinical finding language. New ordinary responses under prompt 4.7 require an exact weekly phrase in `weekly_mirror`, exact source matches for every quotation, three nonempty fields, and one generated question across the complete response; verified source quotations are excluded from generated-question counts and unsupported-state-claim checks. Responses have no minimum length and a 180-word maximum. The saved North Star Moment comparison retains its versioned checks for quotations, question form, and hidden comparison metadata. These checks do not establish semantic accuracy or correct temporal attribution. Historical responses retain their original validation rules. The same checks can produce a separate batch report. | numbered validation level, automated checks |
...
| **Future human calibration of the AI review** | An unexecuted study in which human reviewers rate Coach Digest responses and compare those ratings with Coach Digest Evals. This work is closed outside capstone scope; the historical name does not indicate an active requirement. | numbered evaluation level, human validation when only calibration is meant |
