...
| **North Star Moment** | At most one exact quotation of the user's supportive action within a valid Coach Digest after an eligible closed-week result, between its narrative and reflective question. Active Drift uses writing from before onset; No Active Drift prefers current-week encouragement, then a historical reminder. Insufficient Evidence produces no card. Sources are eligible Journal Entries or user nudge responses with independent availability evidence. The five-Persona demo can compare complete Coach Digest responses generated with and without the selected source context; onboarding preserves its original narrative and question. The feature name is used in product documentation, Inspect, and discreet Persona replay attribution. Personal Experience headings are “A moment in your own words” or “A past moment in your own words,” without the feature name. | proof of recovery, alignment score, best or strongest example |
| **Coach Digest Validations** | The code checks applied when Twinkl creates a Coach Digest response. The checks cover grounded quotes, score jargon, raw Schwartz value leakage, current-state claims, response length, and, from prompt 4.4, recap openings and clinical finding language. The saved North Star Moment comparison also requires a verbatim weekly phrase in `weekly_mirror`, checks every quotation against supplied text, requires one question mark in the reflective question, and rejects comparison metadata in the response. These checks do not establish semantic accuracy or correct temporal attribution. Historical responses retain their original validation rules. The same checks can produce a separate batch report. | numbered validation level, automated checks |
